...
개요: 본 연구는 탄소중립 실현 목표에 기여하기 위한 CO2 지중 저장 기술(CCS)에서의 지중 저장소 안정성을 평가하고 최적의 가스 주입 시나리오를 도출하기 위해 **물리 정보 신경망(PINNs)**, **불확실성 정량화(PCE)**, 강화학습(PPO)을 병합한 차세대 의사결정 시스템을 제안하였다.

---

## 📝 01. 요약 (Abstract)
> 글로벌 탄소 규제 강화에 따라 CCS 기술의 중요성이 대두되고 있으나, 기존 수치 해석 모델과 데이터 기반 딥러닝 모델은 연산 부하가 크고 물리적 비일관성을 내포하는 한계가 있었다. 이를 극복하기 위해 본 연구는 물리 법칙을 내재화한 PINNs를 기반으로 연산 효율을 확보하고, 지질학적 불확실성을 확률적으로 정량화하여 임계 압력 내에서 경제적 수익을 극대화하는 동적 주입 제어 모델을 구축하였다.


---

## 📌 02. 합성 데이터셋 생성 및 전처리 (Synthetic Dataset)
현장의 데이터 부족 한계를 극복하기 위해 3D 지진파 데이터와 1D 시추 로그를 결합한 고정밀 합성 데이터셋을 생성하였다.
...
<p align="center">
  <img src="docs/figures/ricker_wavelet_waveform.png" width="400" alt="Ricker Wavelet Waveform">
</p>
---
